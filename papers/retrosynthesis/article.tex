...
Training a Transformer model is a challenge, and several heuristics have been proposed\cite{TransformerTips}.  The most useful we used in this work are:

\paragraph{Using as bigger batch size as possible.} However, due to our hardware limitations we could not set the batch size greater then 64\footnote{Our first implementation of the model required a lot of memory to deal with masks of reactants and products. Though later we improved the code we still remained this size for consistency of the results.};
...
\paragraph{Averaging weights} during last steps of training or at minima of learning rates.
...
\begin{table}
  ...
  \begin{tabular}{p{2cm}p{1.5cm}p{1.5cm}p{1.5cm}p{8cm}}
    ...
    T2     & & & & Transformer Model trained on both training and validation sets for 1000 epochs.\\
